...
ALL_DATABASE_NAMES=$TARGET_DATABASE_NAMES
if [ "$ALL_DATABASE_NAMES" = "ALL" ]
then
  # get all database names except mysql native
  ExcludeDatabases="Database|information_schema|performance_schema|mysql|sys"
  AllDatabases=`mysql -u$TARGET_DATABASE_USER -h$TARGET_DATABASE_HOST -p$TARGET_DATABASE_PASSWORD -P$TARGET_DATABASE_PORT -e "SHOW DATABASES;" | tr -d " | " | egrep -v $ExcludeDatabases`
  # ALL_DATABASE_NAMES=$AllDatabases  | sed -e "s/\ /\,/g;"
  ALL_DATABASE_NAMES=$AllDatabases
  # echo "$ALL_DATABASE_NAMES"
fi
